...
class SystemDao extends BaseDao {
  ...
  Future<void> insertAnnouncement(Map<String, dynamic> row) async {
    ...
    dbRow['is_active'] = (rawActive == true ||
            rawActive == 1 ||
            rawActive == "1" ||
            rawActive == "true")
        ? 1
        : 0;
    dbRow['is_archived'] = (rawArchived == true ||
            rawArchived == 1 ||
            rawArchived == "1" ||
            rawArchived == "true")
        ? 1
        : 0;
    dbRow['is_deleted'] = (rawDeleted == true ||
            rawDeleted == 1 ||
            rawDeleted == "1" ||
            rawDeleted == "true")
        ? 1
  ...
  Future<List<Map<String, dynamic>>> getAnnouncements() async {
    ...
    return results.map((row) {
      ...
      map['is_active'] =
          (map['is_active'] == 1 || map['is_active'] == true) ? 1 : 0;
      map['is_archived'] =
          (map['is_archived'] == 1 || map['is_archived'] == true) ? 1 : 0;
      map['is_deleted'] =
  ...
  Future<void> insertAlert(Map<String, dynamic> row) async {
    final dbRow = Map<String, dynamic>.from(row);
    dbRow['is_emergency'] = (dbRow['is_emergency'] == true ||
            dbRow['is_emergency'] == 1 ||
            dbRow['isEmergency'] == true ||
            dbRow['isEmergency'] == 1)
        ? 1
        : 0;
    dbRow['is_active'] = (dbRow['is_active'] == true ||
            dbRow['is_active'] == 1 ||
            dbRow['isActive'] == true ||
            dbRow['isActive'] == 1)
        ? 1
  ...
  Future<List<Map<String, dynamic>>> getUnsyncedChatMessages() async {
    return await db
        .query('chat_messages', where: 'is_synced = ?', whereArgs: [0]);
  }

  Future<Map<String, dynamic>?> getChatMessageById(String id) async {
    final results =
        await db.query('chat_messages', where: 'id = ?', whereArgs: [id]);
    return results.isNotEmpty ? results.first : null;
  }
  ...
  Future<void> upsertChatMessage(Map<String, dynamic> data) async {
    ...
    await db.insert('chat_messages', prepared,
        conflictAlgorithm: ConflictAlgorithm.replace);
  }
  ...
  Future<int> updateReminder(Map<String, dynamic> row) async {
    ...
    return await db
        .update('reminders', dbRow, where: 'id = ?', whereArgs: [dbRow['id']]);
  }
  ...
  Future<int> deleteAllReminders(String userId) async {
    return await db
        .delete('reminders', where: 'user_id = ?', whereArgs: [userId]);
  }
