...
class MigrationService {
  ...
  Future<void> performSanityCheck(Database db) async {
    ...
    if (!vitalsColNames.contains('created_at')) {
      await db.execute(
          'ALTER TABLE vitals ADD COLUMN created_at TEXT DEFAULT (datetime('
          'now'
          '))');
    }
    ...
    if (!annCols.any((c) => c['name'] == 'created_at')) {
      await db.execute(
          'ALTER TABLE announcements ADD COLUMN created_at TEXT DEFAULT (datetime('
          'now'
          '))');
    }
    ...
    if (!alertsCols.any((c) => c['name'] == 'created_at')) {
      await db.execute(
          'ALTER TABLE alerts ADD COLUMN created_at TEXT DEFAULT (datetime('
          'now'
          '))');
    }
  }
