...
class LogSyncHandler extends SyncHandler {
  ...
  Future<void> push() async {
    ...
    try {
      ...
      SecurityLogger.info(
          "Sync: Pushing ${unsyncedLogs.length} system logs to Supabase...");
        ...
        try {
          ...
          if (e.toString().contains('42501')) {
            ...
            SecurityLogger.warning(
                "Sync: Log push rejected by Supabase RLS Policy. (Unauthorized 42501)");
            debugPrint("\n${'=' * 60}");
            debugPrint("💡 [RPi Sync Fix Required]");
            debugPrint(
                "The 'system_logs' table is missing an INSERT policy for 'anon'.");
            debugPrint(
                "To fix this and enable cloud logging, run the provided SQL script:");
            debugPrint(
                "file:///C:/Users/Klarence%20Nevado/.gemini/antigravity/brain/1d6893b4-801b-4791-9c58-87d51912d66d/supabase_rls_fix.sql");
            debugPrint("${'=' * 60}\n");
            _isPushing = false;
            return;
          } else {
