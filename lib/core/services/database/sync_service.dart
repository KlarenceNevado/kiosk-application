...
class SyncService with WidgetsBindingObserver {
  ...
  void startSyncLoop() {
    ...
    // 3. Reactive check on network change
    try {
      Connectivity().onConnectivityChanged.listen(
        (ConnectivityResult result) {
          if (result != ConnectivityResult.none) {
            debugPrint("🌐 Network detected. Attempting sync...");
            _attemptSync();
          }
        },
        onError: (error) {
      ...
      debugPrint("⚠️ Could not start connectivity listener: $e");
    }
  }
  ...
  void _subscribeWithRetry({
    ...
    // Safety: Unsubscribe old channel before creating a new one to prevent leaks/stability issues
  ...
  Future<void> _attemptSync() async {
    ...
    try {
      ...
      // 1. Sweep Registered Callbacks (e.g. AuthRepository Patients) UPWARD
      for (final callback in _syncCallbacks) {
        await callback();
      }

      // 2. Sweep SQLite Vitals UPWARD
      final unsyncedVitals = await DatabaseHelper.instance.getUnsyncedRecords();
      for (final vital in unsyncedVitals) {
        final success = await _upsertVitalSign(vital);
        if (success) {
          await DatabaseHelper.instance.markRecordAsSynced(vital.id);
        }
      }
      // UPLOAD (Local -> Cloud) - Run in parallel but don't let one block the others
      await Future.wait([
        _syncAnnouncements()
            .catchError((e) => debugPrint("Announcements Link Fail: $e")),
        _syncAlerts().catchError((e) => debugPrint("Alerts Link Fail: $e")),
        _syncSchedules()
            .catchError((e) => debugPrint("Schedules Link Fail: $e")),
      ]);

      // DOWNWARD SYNC (Cloud -> Local) - Critical for mobile updates
      await Future.wait([
        _syncDownPatients(useMutex: false)
            .catchError((e) => debugPrint("Patients Link Fail: $e")),
        _syncDownVitals(useMutex: false)
            .catchError((e) => debugPrint("Vitals Link Fail: $e")),
        _syncDownAlerts(useMutex: false)
            .catchError((e) => debugPrint("Alerts Down Link Fail: $e")),
        _syncDownSchedules(useMutex: false)
            .catchError((e) => debugPrint("Schedules Down Link Fail: $e")),
        _syncDownAnnouncements(useMutex: false)
            .catchError((e) => debugPrint("Announcements Down Link Fail: $e")),
      ]);
    } catch (e) {
  ...
  Future<void> _syncAnnouncements() async {
    ...
    for (final row in unsynced) {
      try {
        ...
        await DatabaseHelper.instance.markAnnouncementAsSynced(row['id']);
        debugPrint("✅ Background Sync: Announcement '${row['title']}' pushed.");
      } catch (e) {
        debugPrint("⚠️ Background Sync: Failed to push announcement: $e");
      }
    }
  }
  ...
  Future<void> _syncAlerts() async {
    ...
    for (final row in unsynced) {
      try {
        ...
        await DatabaseHelper.instance.markAlertAsSynced(row['id']);
        debugPrint("✅ Background Sync: Alert pushed.");
      } catch (e) {
        debugPrint("⚠️ Background Sync: Failed to push alert: $e");
      }
    }
  }
  ...
  Future<void> _syncSchedules() async {
    ...
    for (final row in unsynced) {
      try {
        ...
        await DatabaseHelper.instance.markScheduleAsSynced(row['id']);
        debugPrint("✅ Background Sync: Schedule pushed.");
      } catch (e) {
        debugPrint("⚠️ Background Sync: Failed to push schedule: $e");
      }
    }
  }
  ...
  Future<void> _syncDownPatients({bool useMutex = true}) async {
    ...
    try {
      ...
      for (var row in cloudPatients) {
        final user = User.fromMap(row).copyWith(isSynced: true);
  ...
  Future<void> _syncDownVitals({bool useMutex = true}) async {
    ...
    try {
      ...
      for (var row in cloudVitals) {
        ...
        await DatabaseHelper.instance.insertVitalSign(vitalMap);
        latestTimestamp = row['updated_at'];
      }
    ...
    try {
      ...
      for (var row in cloudSchedules) {
        await DatabaseHelper.instance.insertSchedule({
          ...
          'colorValue': row['color_value'] ?? 0xFF000000,
          'updated_at': row['updated_at'],
          'is_deleted': row['is_deleted'] == true ? 1 : 0,
          'is_synced': 1,
        });
    ...
    try {
      ...
      for (var row in cloudAnnouncements) {
        ...
        await DatabaseHelper.instance.insertAnnouncement({
          ...
          'content': row['content'] ?? '',
          'targetGroup': row['target_group'] ?? 'all',
  ...
  Future<User?> createPatient(User user) async {
    ...
    final Map<String, dynamic> supabaseData = {
      ...
      'pin_code': user.pinCode, // RESTORED: Needed for mobile app login
      'date_of_birth': user.dateOfBirth.toIso8601String(),
      'gender': user.gender,
      'parent_id': user.parentId,
      'is_active': user.isActive, // Add explicit active state
      'updated_at':
          DateTime.now().toIso8601String(), // Ensure cloud has a timestamp
    };

    try {
      debugPrint("🌐 Pushing patient ${user.fullName} to Supabase...");
      ...
      debugPrint("✅ Patient ${user.fullName} synced successfully.");
      return syncedUser;
    } catch (e) {
      debugPrint("❌ Supabase push failed for ${user.fullName}: $e");
  ...
  Future<bool> updatePatient(User user) async {
    try {
      ...
      debugPrint("✅ Patient update synced: ${user.fullName}");
      return true;
    } catch (e) {
  ...
  Future<bool> deletePatient(String userId) async {
    try {
      ...
      debugPrint("✅ Patient soft-deleted from Cloud: $userId");
      return true;
    } catch (e) {
      debugPrint("⚠️ Cloud delete failed (Offline): $e");
      await DatabaseHelper.instance.deletePatient(userId);
      return false;
    }
  }
  ...
  // Mobile App Autocomplete Search
  ...
  Future<void> deleteScheduleCloud(String id) async {
    try {
      ...
      debugPrint("✅ Schedule deleted from Supabase.");
    } catch (e) {
  ...
  Future<void> deleteAnnouncement(String id) async {
    try {
      ...
      debugPrint("✅ Announcement deleted from Supabase.");
    } catch (e) {
  ...
  Future<List<Map<String, dynamic>>> fetchAnnouncements(
    ...
    try {
      ...
      var filtered = all.where((a) {
        ...
        final isActive = a['isActive'] == 1 || a['isActive'] == true;
      ...
      if (currentUser != null) {
        ...
        filtered = filtered.where((a) {
          final target = (a['targetGroup'] ?? a['target_group'])
                  ?.toString()
                  .toUpperCase() ??
              'ALL';
