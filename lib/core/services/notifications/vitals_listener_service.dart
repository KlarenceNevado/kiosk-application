...
class VitalsListenerService {
  static final VitalsListenerService _instance = VitalsListenerService._internal();
  ...
  void startListening({
    ...
    debugPrint("📡 Starting Vitals Listener for Family [${familyIds.join(', ')}] (via SyncService)...");
    ...
    _syncSubscription = SyncService().newVitalStream.listen((data) {
      ...
      if (_monitoredUserIds.contains(recordUserId)) {
        debugPrint("🆕 New Cloud Vital detected for family member: $recordUserId");
        ...
        NotificationService().showInstantNotification(
          ...
          title: title,
          body: "A new health reading has been processed for your family account.",
        );
      }
    });
