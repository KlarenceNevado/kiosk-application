...
class ConfigService {
  ...
  Future<void> loadSettings() async {
    ...
    _printerPort = prefs.getString(_keyPrinterPort) ?? _printerPort;
    _facilityName = prefs.getString(_keyFacilityName) ?? _facilityName;
  }

  /// Save new settings
  Future<void> updateSettings({String? ip, String? port, String? name}) async {
    final prefs = await SharedPreferences.getInstance();
    if (ip != null) {
    ...
    if (port != null) {
      _printerPort = port;
      await prefs.setString(_keyPrinterPort, port);
    }
    if (name != null) {
      _facilityName = name;
