...
enum PowerMode { active, eco, deepSleep }
...
class PowerManagerService {
  ...
  void updateBatteryStatus(double voltage) {
    ...
    if (voltage < 11.5 && _currentMode != PowerMode.deepSleep) {
      debugPrint(
          "⚠️ [PowerManager] CRITICAL BATTERY ($voltage V). Forcing Deep Sleep.");
  ...
  void _resetTimer() {
    ...
    _inactivityTimer =
        Timer(const Duration(seconds: dimThreshold), _onIdleThresholdReached);
  }
  ...
  void _setMode(PowerMode mode) {
    ...
    SystemLogService().logAction(
        ...
        severity: 'INFO',
        sensorFailures:
            'System entered $mode mode. Battery: $_currentVoltage V (${batteryPercentage.toInt()}%)');
  }
  ...
  Future<void> _onIdleThresholdReached() async {
    ...
    _inactivityTimer = Timer(
        const Duration(seconds: ecoModeThreshold - dimThreshold), () async {
      ...
      // Schedule Stage 3
      _inactivityTimer =
          Timer(const Duration(seconds: deepSleepThreshold - ecoModeThreshold),
              () async {
