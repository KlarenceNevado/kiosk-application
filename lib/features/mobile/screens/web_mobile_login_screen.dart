...
import '../../../../core/config/web_routes.dart';

/// Web-safe Mobile Login Screen.
/// Uses Supabase directly for patient search and WebAuthRepository for login.
class WebMobileLoginScreen extends StatefulWidget {
  const WebMobileLoginScreen({super.key});

  @override
  State<WebMobileLoginScreen> createState() => _WebMobileLoginScreenState();
}

class _WebMobileLoginScreenState extends State<WebMobileLoginScreen> {
  ...
  void _onSearchChanged() {
    ...
    _debounce = Timer(const Duration(milliseconds: 500), () async {
      try {
        // Search Supabase directly (web-safe, no SyncService)
        ...
        final results = data.map((row) => User.fromMap(row)).toList();
  ...
  Future<void> _login() async {
    ...
    try {
      ...
      if (mounted) {
        context.go(WebAppRoutes.patientHome);
      }
    } catch (e) {
      if (mounted) {
        setState(() {
