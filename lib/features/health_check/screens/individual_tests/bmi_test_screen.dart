...
class _BmiTestScreenState extends State<BmiTestScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    // HARDENING: Auto-lock
    if (_stage == 2 &&
        provider.isVitalStable(SensorType.weight) &&
        provider.weightKg > 5) {
      ...
      body: Container(
        ...
        child: AnimatedSwitcher(
          ...
          child: _stage == 0
              ...
              : _stage == 1
                  ? _buildReadyWait(themeColor)
                  : _stage == 4
                      ? _buildErrorView(themeColor)
                      : _buildMeasurementView(provider, themeColor),
        ),
      ),
    );
  }
  ...
  Widget _buildReadyWait(Color color) {
    return Column(
      ...
      children: [
        ...
        const Text("Stand consistently on the platform scale.",
            style: TextStyle(
              ...
              TweenAnimationBuilder(
                ...
                builder: (context, val, child) => Container(
                  ...
                  decoration: BoxDecoration(
                      shape: BoxShape.circle,
                      border: Border.all(
                          color: color.withValues(alpha: 0.1), width: 3)),
                ),
              ),
            SizedBox(
              width: 300,
        ...
        if (isDone)
          FlowAnimatedButton(
            child: ElevatedButton(
              ...
              child: const Text("Save & Record BMI",
                  style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold)),
            ),
          ),
      ],
  ...
  Widget _buildErrorView(Color color) {
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 24),
        const Text("Scale Error",
            style: TextStyle(
                fontSize: 32,
                fontWeight: FontWeight.w900,
                color: AppColors.brandDark)),
        const SizedBox(height: 12),
        const Text("Could not get a stable reading. Please try again.",
    ...
    return Column(
      children: [
        Container(
          ...
          child: Text(
              "${provider.bmiCategory.toUpperCase()}  (BMI: ${provider.bmi.toStringAsFixed(1)})",
              style: TextStyle(
                  color: bmiColor, fontSize: 20, fontWeight: FontWeight.w900)),
        ),
      ],
    );
  }
