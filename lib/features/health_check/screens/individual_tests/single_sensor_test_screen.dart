...
class _SingleSensorTestScreenState extends State<SingleSensorTestScreen>
  ...
  void _startScan() {
    ...
    _timeoutTimer = Timer(
        Duration(
            seconds: widget.type == TestSensorType.bloodPressure ? 45 : 15),
        () {
    ...
    _simTimer = Timer.periodic(const Duration(milliseconds: 200), (timer) {
      ...
      if (widget.type == TestSensorType.temperature) {
        provider.setTemperature(36.1 + Random().nextDouble() * 0.5);
          ...
          widget.type == TestSensorType.oxygen) {
        provider.setPulseOx(
            72 + Random().nextInt(10), 97 + Random().nextInt(3));
      } else if (widget.type == TestSensorType.bloodPressure) {
        if (ticks >= 20) {
          provider.setBloodPressure(
              120 + Random().nextInt(10), 80 + Random().nextInt(5));
        }
      }
  ...
  void _lockValueAndFinish() {
    ...
    setState(() {
      ...
      switch (widget.type) {
        case TestSensorType.temperature:
          _lockedResult = provider.currentTemp.toStringAsFixed(1);
          break;
        case TestSensorType.heartRate:
          _lockedResult = "${provider.currentHeartRate}";
          break;
        case TestSensorType.oxygen:
          _lockedResult = "${provider.currentSpO2}";
          break;
        case TestSensorType.bloodPressure:
          _lockedResult =
  ...
  Future<void> _saveAndFinish() async {
    ...
    final VitalSigns record = VitalSigns(
      ...
      heartRate: widget.type == TestSensorType.heartRate
          ? provider.currentHeartRate
          : 0,
      ...
      temperature: widget.type == TestSensorType.temperature
          ? provider.currentTemp
          : 0.0,
    );
  ...
  Widget build(BuildContext context) {
    ...
    if (_viewState == 1) {
      ...
      switch (widget.type) {
        case TestSensorType.temperature:
          shouldFinish = provider.isVitalStable(hw.SensorType.thermometer) &&
              provider.currentTemp > 34;
          break;
        case TestSensorType.heartRate:
        case TestSensorType.oxygen:
          shouldFinish = provider.isVitalStable(hw.SensorType.oximeter) &&
              provider.currentHeartRate > 40;
          break;
        case TestSensorType.bloodPressure:
          shouldFinish = provider.currentSystolic > 30;
          break;
      }
    ...
    if (_viewState == 1) {
      switch (widget.type) {
        case TestSensorType.temperature:
          displayValue = provider.currentTemp > 0
              ? provider.currentTemp.toStringAsFixed(1)
              : "READING";
          break;
        case TestSensorType.heartRate:
          displayValue = provider.currentHeartRate > 0
              ? "${provider.currentHeartRate}"
              : "READING";
          break;
        case TestSensorType.oxygen:
          displayValue =
              provider.currentSpO2 > 0 ? "${provider.currentSpO2}" : "READING";
          break;
        case TestSensorType.bloodPressure:
          displayValue = provider.currentSystolic > 0
              ? "${provider.currentSystolic}/${provider.currentDiastolic}"
              : "INFLATING";
          break;
      }
          ...
          child: ConstrainedBox(
            ...
            child: AnimatedSwitcher(
              ...
              child: _viewState == 0
                  ...
                  : _viewState == 3
                      ? _buildErrorView(testColor)
                      : _buildMeasurementView(testColor, icon, displayValue),
            ),
          ),
        ),
      ),
  ...
  Widget _buildPrepView(Color color, IconData icon) {
    return Column(
      ...
      children: [
        ...
        const Text("Precautionary Stage",
            style: TextStyle(
                fontSize: 24,
      ...
      children: [
                  ...
                  (index) => TweenAnimationBuilder(
                        ...
                        builder: (context, val, child) => Container(
                          ...
                          decoration: BoxDecoration(
                              shape: BoxShape.circle,
                              border: Border.all(
                                  color: color.withValues(alpha: 0.15 / val),
                                  width: 2)),
                        ),
                      )),
            SizedBox(
              ...
              child: Column(
                ...
                children: [
                  if (widget.type == TestSensorType.heartRate && !isDone)
                    ...
                    Icon(icon,
                        color: isDone ? AppColors.brandGreen : color, size: 28),
                  const SizedBox(height: 6),
                  Text(value,
                      style: TextStyle(
                          fontSize: value.length > 5 ? 32 : 52,
                          fontWeight: FontWeight.w900,
                          color: isDone ? AppColors.brandGreen : color,
                          letterSpacing: -1)),
                  Text(unit,
        ...
        Container(
          ...
          decoration: BoxDecoration(
              ...
              borderRadius: BorderRadius.circular(50)),
          child: Text(isDone ? "TEST COMPLETE" : "STABILIZING...",
              style: TextStyle(
                  fontSize: 14,
                  fontWeight: FontWeight.bold,
                  color: isDone ? AppColors.brandGreen : color)),
        ...
        if (isDone)
          FlowAnimatedButton(
            child: ElevatedButton(
              ...
              child: const Text("Save & Record Result",
                  style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
            ),
          ),
      ],
  ...
  Widget _buildErrorView(Color color) {
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 24),
        const Text("Measurement Failed",
            style: TextStyle(
                fontSize: 28,
                fontWeight: FontWeight.w900,
                color: AppColors.brandDark)),
        const SizedBox(height: 12),
        const Text("The sensor timed out. Please try again.",
            textAlign: TextAlign.center,
        ...
        ElevatedButton(
          ...
          style: ElevatedButton.styleFrom(
              ...
              shape: RoundedRectangleBorder(
                  borderRadius: BorderRadius.circular(50))),
          child: const Text("Retry Measurement",
              style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
        ),
      ],
    );
  ...
  String _getInstruction() {
    switch (widget.type) {
      case TestSensorType.temperature:
        return "Forehead 5cm from the sensor. Remove hair/hats.";
      case TestSensorType.bloodPressure:
        return "Cuff on left arm, sit straight, and stay still.";
      case TestSensorType.heartRate:
        return "Insert index finger into the pulse clip.";
      case TestSensorType.oxygen:
        return "Insert index finger into the pulse clip.";
    }
  }
