...
void main() async {
  ...
  try {
    final patients = await db
        .rawQuery("SELECT COUNT(*) as count FROM patients WHERE is_synced = 0");
  ...
  // 2. Unsynced Vitals
  try {
    final vitals = await db
        .rawQuery("SELECT COUNT(*) as count FROM vitals WHERE is_synced = 0");
  ...
  try {
    final blocked = await db.rawQuery(
        "SELECT table_name, record_id, last_error FROM sync_metadata WHERE is_blocked = 1");
    if (blocked.isEmpty) {
      ...
      for (var row in blocked) {
        print(
            "  - Table: ${row['table_name']}, ID: ${row['record_id']}, Error: ${row['last_error']}");
      }
    }
  } catch (e) {
    print(
        "Sync Metadata Info: ${e.toString().contains('no such table') ? 'No blocked records table yet.' : e}");
  }
